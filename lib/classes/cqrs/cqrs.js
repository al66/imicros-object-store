...
class Repository {
    ...
    async getInstance(instanceId) {
        ...
        this.#setVersion(instance, versionFromEvents);
    ...
    async persist(instanceId, events = [], options = {}) {
        ...
        const expectedVersion = options.expectedVersion !== undefined ? options.expectedVersion : this.#getVersion(current);
        ...
        this.#setVersion(current, currentVersion);
    ...
    #setVersion(instance, version) {
        if (instance && typeof instance === "object") {
            this.versions.set(instance, Number(version || 0));
        }
    }

    #getVersion(instance) {
        if (!instance || typeof instance !== "object") return 0;
        return this.versions.get(instance) || 0;
    }
}
...
function createRepositoryMixin(options = {}) {
    return {
        created() {
            let db = this.settings && this.settings.database;
